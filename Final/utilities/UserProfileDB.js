...
//function to add/edit a connection to the list
var addNewConnection = async function(connection){
    var connFind = [];
    // await conn.find({connectionTopic: connection.connectionTopic, connectionName: connection.connectionName, host: connection.host, date: connection.date, startTime: connection.startTime})
    ...
    if(connFind.length == 0){
        ...
        await newConn.save(async function(err, res){
            ...
            result = await res;
            //result = res;
            //console.log('result in userProfileDB = ', result);
...
//function to get the entries by connectionID
